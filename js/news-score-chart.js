...
function ensureChartElements() {
  ...
  if (!container) {
    console.warn("A newsScoreChartContainer nem található.");
    return null;
  }

  let canvas = document.getElementById("newsScoreChart");

  if (!canvas) {
    const wrapper = document.createElement("div");
    wrapper.className = "score-chart-wrap";
    ...
    canvas.id = "newsScoreChart";

    wrapper.appendChild(canvas);

    const note = container.querySelector(".score-note");
    if (note) {
      container.insertBefore(wrapper, note);
    } else {
      container.appendChild(wrapper);
    }
  }

  let summary = document.getElementById("newsScoreSummary");

  if (!summary) {
    summary = document.createElement("div");
    summary.id = "newsScoreSummary";

    const note = container.querySelector(".score-note");
    if (note) {
      container.insertBefore(summary, note);
    } else {
...
function getBarColor(value) {
  if (value > 0) return "rgba(34, 197, 94, 0.75)";
  if (value < 0) return "rgba(239, 68, 68, 0.75)";
  return "rgba(148, 163, 184, 0.65)";
}

function getBarBorderColor(value) {
  if (value > 0) return "rgba(134, 239, 172, 1)";
  if (value < 0) return "rgba(252, 165, 165, 1)";
...
function renderNewsScoreChart(processedData) {
  const canvas = ensureChartElements();
  ...
  if (typeof Chart === "undefined") {
    throw new Error("A Chart.js nem töltődött be. Ellenőrizd, hogy az index.html tartalmazza-e a Chart.js scriptet.");
  }
      ...
      datasets: [
        {
          ...
          data: dailyScores,
          yAxisID: "y",
          backgroundColor: dailyScores.map(getBarColor),
          borderColor: dailyScores.map(getBarBorderColor),
          borderWidth: 1,
          borderRadius: 6
        },
        {
  ...
  try {
    ...
    if (container) {
      container.innerHTML = `
        ...
        <p style="color:#cbd5e1;font-size:11px;line-height:1.45;">
          Hiba: ${String(error.message || error)}
        </p>
      `;
    }
  }
